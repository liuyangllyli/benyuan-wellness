...
本元养生是以**「生活节律」**为轴心的健康管理应用，持续回答用户：**「此刻，我最适合为身体做的一件事是什么？」**
...
- **首页（此刻）**：可进入炼体 `/practice`、食养 `/diet`、明理 `/knowledge`、归元 `/records`、设置 `/settings`；可进入觉察 `/reflection`。
...
| 组件 | 职责 |
|------|------|
| **BottomNav** | 6 标签（此刻/知己/炼体/食养/明理/归元）；在 `/onboarding`、`/calm` 等路由时隐藏。 |
