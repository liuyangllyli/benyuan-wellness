...
> 含 6 张 Mermaid 流程图：用户主流程、数据架构、组件依赖、AI 接口契约、数据飞轮、关键文件索引。Cursor 可直接引用。  
> **与 `docs/architecture.md` 一致；最后对齐日期：2026-02-21。项目名称：本元养生 / BenYuan Wellness。**
...
已按与参考原型的逐条回复以**实现为准**统一（2026-02-21）。9 条结论见 **`docs/architecture.md`** 开头的逐条结论表。本文件与 **`docs/architecture.md`** 内容一致，可作为 Cursor 事实来源。**项目名称**：本元养生 / BenYuan Wellness。
